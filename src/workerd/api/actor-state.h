...
public:
  ...
  // the event to take longer than the specified timeout, if set.
  void setHibernatableWebSocketEventTimeout(kj::Maybe<uint32_t> timeoutMs);

  // Get the currently set hiberntable websocket event timeout if set, or kj::none if not.
  kj::Maybe<int> getHibernatableWebSocketEventTimeout();
