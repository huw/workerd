...
struct CallResults {
  union {
    ...
    exception @1 :JsValue;
    # An exception that was encountered while attempting to execute call().
    # This could be a JS exception thrown by the function, a failure to serialize the return value,
    # or some other KJ exception that was thrown by call().
  }
}

interface JsRpcTarget {
  call @0 (methodName :Text, serializedArgs :JsValue) -> CallResults;
